...
from .data_types_for_edit import Author, AuthorSource, CitesPerYear, Journal, Publication, PublicationSource
...
class _Scholarly:
    ...
    def search_pubs(self,
        ...
        citations=citations, year_low=year_low, year_high=year_high, sort_by=sort_by, include_last_year=include_last_year, start_index=start_index)
        #url = self._construct_url(_PUBSEARCH.format(requests.utils.quote(query)), start_index=start_index)
    ...
    # def _construct_url(self, baseurl: str, start_index: int = 0)-> str:
